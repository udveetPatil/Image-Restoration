...
> **Pre-computed results are already included in this repository.**
> All figures in `results/figures/` and metrics in `results/metrics/` were generated by running `main.m` on the standard `cameraman.tif` test image. You can browse them directly on GitHub without running any code.
...
- **User-supplied images** — pick any image at runtime, or use the default `cameraman.tif`
...
│   └── load_image.m                # User image input + RGB→gray handling
├── experiments/
...
├── ai/
│   └── ai_comparison.m             # Pretrained DnCNN denoising network
└── results/
    ├── figures/                    # Pre-generated PNG outputs (cameraman.tif)
    └── metrics/                    # Pre-generated CSV + MAT outputs
```
...
You can run this project in two ways: **directly from your browser** (no installation) or **locally** in MATLAB.

### Option A — Run in MATLAB Online (recommended, zero setup)

1. Click the badge at the top of this page, or use this link:

   **[Open in MATLAB Online](https://matlab.mathworks.com/open/github/v1?repo=udveetPatil/Image-Restoration)**

2. Sign in with a free [MathWorks account](https://www.mathworks.com/mwaccount/register) if prompted.
3. MATLAB Online will clone this repository and open the project automatically (takes ~30 seconds).
...
5. When prompted, choose:
   - `1` to run on the default `cameraman.tif`
   - `2` to upload and use your own image

All figures and metrics regenerate in your session. No files are saved back to GitHub — the original results stay intact.

**Free-tier limits:** ~20 hours of compute/month, 5 GB storage. More than enough for exploration.
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/udveetPatil/Image-Restoration.git
   cd Image-Restoration
   ```

2. Open MATLAB and change into the project root:
...
4. Choose the default image or pick your own when prompted.
...
When you run `main`, you'll be prompted:

```
Choose image source:
  1. Use default (cameraman.tif)
  2. Pick an image file
Enter 1 or 2 [default=1]:
```

Choosing `2` opens a file dialog. Any common format works (`.png`, `.jpg`, `.tif`, `.bmp`). Color images are automatically converted to grayscale using luminance weighting (`rgb2gray`). Images larger than 512 px on either side are resized automatically.
